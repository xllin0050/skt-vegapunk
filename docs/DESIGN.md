...
## 9. SktVegapunk Web UI

### 9.1 專案結構

新增 `SktVegapunk.Web` 專案，與 `SktVegapunk.Console` 並存。
共用 `SktVegapunk.Core`，Console 完全不動。

```
SktVegapunk.Web/
├── SktVegapunk.Web.csproj
├── Program.cs              ← Minimal API endpoints + 靜態檔案
└── wwwroot/
    └── index.html          ← 單頁應用（vanilla JS）
```
...
```
┌──────────────────────────────────────────────────────────┐
│  navbar: SktVegapunk  [dark #222126]                     │
├─────────────────┬────────────────────────────────────────┤
│  LEFT  280px    │  RIGHT  flex-1                         │
│  [dark #2B2F36] │  [dark #1E2026]                        │
│                 │                                        │
│  輸出模式        │  ┌─ 執行中 ──────────────────────────┐ │
│  ○ 規格拆解      │  │  terminal log（SSE 串流）          │ │
│  ○ 程式碼遷移    │  │  monospace, Crypto Green on dark  │ │
│                 │  └───────────────────────────────────┘ │
│  AI 模型         │                                        │
│  [dropdown]     │  ┌─ 執行完成 ─────────────────────────┐ │
│                 │  │ ┌─ 檔案樹 ──┬─ Markdown 預覽 ────┐  │ │
│  Source 路徑     │  │ │ INDEX.md  │  # Spec Artifact   │  │ │
│  [input][瀏覽]   │  │ │ report.md │  Index             │  │ │
│                 │  │ │ unresolved│  ...                │  │ │
│  Output 路徑     │  │ │ inferred  │                    │  │ │
│  [input][瀏覽]   │  │ └───────────┴────────────────────┘  │ │
│                 │  └───────────────────────────────────┘ │
...
| 欄位 | 元件 | 說明 |
|------|------|------|
| 輸出模式 | radio group | `spec`（規格拆解）/ `migration`（程式碼遷移） |
| AI 模型 | `<select>` | 從 `GET /api/config/models` 取得可用模型清單 |
| Source 路徑 | text input + 瀏覽按鈕 | 瀏覽按鈕開啟 server-side 目錄選擇器 |
| Output 路徑 | text input + 瀏覽按鈕 | 同上 |
| 執行 | primary button（Binance Yellow） | 送出後 disabled 直到串流結束 |

`migration` 模式需額外顯示：
- Output 檔案路徑（.cs）
- Target project 路徑（.csproj / .sln）

### 9.4 Right Panel 狀態

**狀態一：待機**
- 顯示使用說明文字（Slate `#848E9C`）
- 提示選擇路徑後按執行

**狀態二：執行中**
- terminal 區塊（背景 `#0D0D0D`，文字 Crypto Green `#0ECB81`，字型 monospace）
- 每行 log 透過 `EventSource` 即時附加
- 頂部 spinner + 「執行中...」標籤

**狀態三：執行完成**
- terminal 區塊縮為摺疊面板（可展開）
- 左側：artifact 檔案樹（遞迴列出 output 路徑下的 `.md` 檔）
- 右側：點擊檔案後 fetch 內容，以 `marked.js` 渲染 Markdown
- 成功顯示 Crypto Green badge，失敗顯示 Crypto Red badge

### 9.5 Backend API

```
GET  /api/config/models
     → string[]   可用 AI 模型清單（來自 appsettings Agent:ModelName 與候選清單）

GET  /api/dirs?path={dir}
     → { path, entries: [{ name, fullPath, isDir }] }
     僅列出目錄，供路徑選擇器使用

POST /api/run
     body: { mode, model, sourcePath, outputPath, ... }
     → { runId: string }

GET  /api/run/{runId}/stream
     Content-Type: text/event-stream
     每行 stdout/stderr 送一個 SSE event
     結束時送 event: done\ndata: {exitCode}

GET  /api/artifacts?base={outputPath}
     → { files: [{ name, relativePath, fullPath }] }
     遞迴列出 .md 檔

GET  /api/artifacts/content?path={fullPath}
     → text/plain   Markdown 原始內容
```
...
| 項目 | 選擇 | 理由 |
|------|------|------|
| JS | Vanilla JS（ES2022） | 無 build 工具，直接 `<script type="module">` |
| Markdown 渲染 | `marked.js`（CDN） | 輕量，無依賴 |
| Code highlight | `highlight.js`（CDN） | 搭配 marked 的 `highlight` callback |
| SSE | 原生 `EventSource` | 瀏覽器內建，無需套件 |
| 樣式 | inline CSS + CSS variables | 遵循本文件 Design System |

### 9.7 配色套用

沿用本文件 Design System，關鍵對應：

| 區域 | 色值 |
|------|------|
| Navbar | `#222126` |
| Left panel | `#2B2F36` |
| Main background | `#1E2026` |
| Terminal 背景 | `#0D0D0D` |
| Terminal 文字 | `#0ECB81`（Crypto Green） |
| 執行按鈕 | `#F0B90B`（Binance Yellow）文字 `#1E2026` |
| 選中檔案 | `#F0B90B` 左側 border，背景 `rgba(240,185,11,0.08)` |
| Markdown 文字 | `#E6E8EA` |
| Markdown code block | `#2B2F36` 背景 |
